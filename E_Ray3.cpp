...
void E_Ray3::init(EScript::Namespace & lib) {
	...
	ES_MFUN(typeObject,const Ray3,"distance",1,1,thisObj->distance(parameter[0].to<const Vec3&>(rt)))
	...
	ES_MFUN(typeObject,const Ray3,"getClosestPoint",1,1,std::move(thisObj->getClosestPoint(parameter[0].to<const Vec3&>(rt))))
	...
	ES_MFUN(typeObject,Ray3,"setDirection",1,1,(thisObj->setDirection(parameter[0].to<const Vec3&>(rt)),thisEObj))

	//!	[ESMF] self ray3.setOrigin(Vec3)
	ES_MFUN(typeObject,Ray3,"setOrigin",1,1,(thisObj->setOrigin(parameter[0].to<const Vec3&>(rt)),thisEObj))
}
